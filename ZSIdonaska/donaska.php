...
<div class="container">
  ...
  <div class="content">
    ...
    <?php $query  = "SELECT pizzaObj,id FROM pizza";
          $result = mysql_query($query);
          while($row = mysql_fetch_row($result)){
		      $pizza=unserialize($row[0]);
			  $foto=$pizza->getFoto();
			  $nazov=$pizza->getNazov();
			  $zlozenie=$pizza->getZlozenie();
			  $mnozstvo=$pizza->getGramaz();
			  $cena=$pizza->getCena();
			  $id=$row[1];
	?>
    
    <h2><img src="images/pizza1.jpg" alt="Foto" width="250" height="250" align="left" class="image" /></h2>
    <h2>&nbsp;</h2>
    <h3><? echo $nazov; ?> </h3>
<p> <strong>Zloženie</strong>: <em><? echo $zlozenie ?> </em></p>
    <p><strong>Množstvo</strong>: <em><? echo $mnozstvo;?></em></p>
    <p><strong>Cena</strong>: <em><? echo $cena; ?> </em></p>
    <form id="form1" name="form1" method="post" action="">
	<input type="hidden" name="pizza_id" value="<? echo $id; ?> "/>
      <select name="vyber_pizza" id="vyber_pizza">
        <option value="pizza" selected="selected"><? echo "$nazov $gramaz g - $cena €"; ?> </option>
      </select>
    počet 
